...
# Instantiate the working list for transferring state
reproduction_list = []
...
# Initiate seed retrieval from db, this is placed at the
# first row of output by insert_root method
retrieve_seed()

# Instantiate the working list from db
grid = []

# build_grid method parses the dataframe imported from db
# and prepares the input for search_eight method
build_grid()

# search_eight method analyzes the nodes on each of 8 points
# around the cell which is being evaluated for life and populates
# the reproduction_list for integrations in grid each generation
search_eight()
